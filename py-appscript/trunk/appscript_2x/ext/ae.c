...
static PyObject *AEDesc_AEDeterminePermissionToAutomateTarget(AEDescObject *_self, PyObject *_args) // thread-safe
{
	PyObject *_res = NULL;
	...
	if (AEDeterminePermissionToAutomateTarget == NULL) {
		PyErr_SetString(PyExc_NotImplementedError,
						"aem.ae.AEDesc.permissiontoautomate isn't available in 10.13 or earlier.");
		return NULL;
	}
	if (!PyArg_ParseTuple(_args, "O&O&b",
	                      ...
	                      &askUserIfNeeded))
		return NULL;
	_err = AEDeterminePermissionToAutomateTarget(&_self->ob_itself, 
												 theAEEventClass, 
												 theAEEventID, 
												 askUserIfNeeded);
	if (_err != noErr) return AE_MacOSError(_err);
...
static PyMethodDef AEDesc_methods[] = {
	...
	{"permissiontoautomate", (PyCFunction)AEDesc_AEDeterminePermissionToAutomateTarget, 1, PyDoc_STR(
		"D.permissiontoautomate(AEEventClass theAEEventClass,\n"
		"                       AEEventID theAEEventID,\n"
		"                       Boolean askUserIfNeeded)\n"
		"Throws if current process is not authorized to send an Apple event\n"
		"to the target application. Descriptor must be an AEAddressDesc.\n"
		"Errors include:\n"
		"errAEEventNotPermitted (-1743): the user has declined permission.\n"
    	"errAEEventWouldRequireUserConsent (-1744): user consent is required\n"
    	"  for this, but the user has not yet been prompted for it. You need\n"
    	"  to pass False for askUserIfNeeded to get this.\n"
    	"procNotFound (-600): the specified app is not currently running.\n"
    	"Throws NotImplementedError on 10.13 and earlier.")},
