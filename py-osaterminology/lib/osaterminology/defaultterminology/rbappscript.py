...
properties = [
		('class_', b'pcls'), # used as a key in AERecord structures that have a custom class; also some apps (e.g. Jaguar Finder) may omit it from their dictionaries despite using it
		('properties_', b'pALL'),
		('id_', b'ID  '), # some apps (e.g. iTunes) may omit 'id' property from terminology despite using it
]
